...
from .lender_recommendation_agent import LenderRecommendationAgent
...
class LoanRiskAdvisorAgent:
    def __init__(self, groq_api_key: str = None):
        ...
        try:
            self.lender_agent = LenderRecommendationAgent(self.groq_api_key)
        except Exception as e:
            print(f"Warning: Could not initialize lender recommendation agent: {e}")
            self.lender_agent = None
...
"""
    ...
    def _categorize_risk(self, credit_score: float) -> str:
        """Categorize risk level based on credit score (300-900 range)"""
        if credit_score >= 750:
    ...
    def _generate_approval_recommendation(self, credit_score: float, loan_type: str, loan_amount: float) -> str:
        """Generate approval recommendation based on credit score (300-900 range)"""
        if credit_score >= 750:
            return "APPROVED - Excellent credit profile, preferential terms available"
        elif credit_score >= 650:
            return "APPROVED - Good credit profile, standard terms recommended"
        elif credit_score >= 550:
            return "CONDITIONAL APPROVAL - Fair credit, enhanced documentation required"
        elif credit_score >= 450:
            return "CONDITIONAL APPROVAL - Below average credit, strict conditions apply"
        else:
            return "REJECTED - Poor credit profile, alternative products suggested"
    ...
    def get_comprehensive_loan_advice_with_lenders(self, user_data: Dict[str, Any], loan_request: Dict[str, Any], 
                                                  credit_result: Dict[str, Any] = None, language: str = "english") -> Dict[str, Any]:
        """
        Provide comprehensive loan advice including nearby lender recommendations
        
        Args:
            user_data: User profile data
            loan_request: Loan requirements (amount, type, purpose)
            credit_result: Credit scoring results (optional)
            language: Response language
            
        Returns:
            Dict containing loan advice and lender recommendations with map
        """
        
        # Step 1: Assess loan risk
        if credit_result and 'credit_score' in credit_result:
            # Use credit score for risk categorization 
            ...
            risk_assessment = {
                "overall_risk_score": credit_score,
                "risk_category": risk_category,
                "individual_scores": {},  # Could add detailed breakdown if needed
                "loan_details": {
                ...
                "approval_recommendation": self._generate_approval_recommendation(credit_score, loan_request.get("type", "personal"), loan_request.get("amount", 0))
            }
        else:
            # Fallback to original risk assessment if no credit score
            risk_assessment = self.assess_loan_risk(user_data, loan_request)
        
        # Step 2: Get loan terms recommendation
        loan_terms = self.recommend_loan_terms(risk_assessment, language)
        
        # Step 3: Get lender recommendations if lender agent is available
        lender_recommendations = None
        lender_analysis = None
        map_html = None
        
        if self.lender_agent:
            try:
                lender_recommendations = self.lender_agent.recommend_lenders(
                    user_data, 
                    loan_request, 
                    max_distance=200
                )
                
                if lender_recommendations.get('recommendations'):
                    lender_analysis = self.lender_agent.generate_lender_analysis(
                        lender_recommendations['recommendations'],
                        loan_request,
                        language
                    )
                    map_html = lender_recommendations.get('map_html')
                    
            except Exception as e:
                print(f"Error getting lender recommendations: {e}")
        
        # Step 4: Generate comprehensive advice
        comprehensive_advice = self._generate_comprehensive_advice(
            user_data, loan_request, risk_assessment, loan_terms, 
            lender_recommendations, language
        )
        
        return {
            "loan_advice": comprehensive_advice,
            "risk_assessment": risk_assessment,
            "recommended_terms": loan_terms,
            "lender_recommendations": lender_recommendations,
            "lender_analysis": lender_analysis,
            "map_html": map_html,
            "approval_recommendation": risk_assessment.get('approval_recommendation', 'REVIEW'),
            "next_steps": self._generate_next_steps(risk_assessment, lender_recommendations, language)
        }
    
    def _generate_comprehensive_advice(self, user_data: Dict[str, Any], loan_request: Dict[str, Any],
                                     risk_assessment: Dict[str, Any], loan_terms: Dict[str, Any],
                                     lender_recommendations: Dict[str, Any], language: str) -> str:
        """Generate comprehensive loan advice combining all factors"""
        
        # Prepare context for AI analysis
        context = {
            "user_profile": {
                "location": user_data.get('village_name', 'Unknown'),
                "occupation": user_data.get('primary_occupation', 'Unknown'),
                "income": user_data.get('monthly_income', 'Unknown'),
                "credit_factors": risk_assessment.get('risk_factors', [])
            },
            "loan_request": loan_request,
            "risk_assessment": {
                "score": risk_assessment.get('overall_risk_score', 0),
                "category": risk_assessment.get('risk_category', 'Unknown'),
                "recommendation": risk_assessment.get('approval_recommendation', 'REVIEW')
            },
            "recommended_terms": loan_terms,
            "available_lenders": len(lender_recommendations.get('recommendations', [])) if lender_recommendations else 0
        }
        
        advice_prompt = f"""
        Provide comprehensive loan advice for a microfinance customer based on this analysis:
        ...
        4. Preparation steps for application
        5. Tips for improving approval chances
        6. Available lender options in the area

        Write in a helpful, encouraging tone suitable for rural customers.
        """
        ...
        try:
            response = self.client.chat.completions.create(
                model=self.model,
                messages=[
                    {"role": "system", "content": "You are a helpful microfinance advisor providing practical guidance to rural customers in India."},
            ...
            print(f"Error generating comprehensive advice: {e}")
            return self._fallback_comprehensive_advice(context)
    
    def _generate_next_steps(self, risk_assessment: Dict[str, Any], lender_recommendations: Dict[str, Any], language: str) -> List[str]:
        """Generate actionable next steps for the user"""
        
        next_steps = []
        approval_rec = risk_assessment.get('approval_recommendation', 'REVIEW')
        
        if approval_rec == 'APPROVE':
            next_steps = [
                "Gather required documents (Aadhaar, bank statements, income proof)",
                "Contact the recommended lenders in your area",
                "Compare interest rates and terms from multiple lenders",
                "Submit loan application with complete documentation",
                "Follow up regularly on application status"
            ]
        elif approval_rec == 'CONDITIONAL_APPROVE':
            next_steps = [
                "Work on improving your credit profile",
                "Consider a smaller loan amount initially",
                "Explore group lending or SHG options",
                "Gather strong collateral or guarantor support",
                "Contact lenders who specialize in your loan type"
            ]
        else:  # REJECT
            next_steps = [
                "Focus on building savings and financial history",
                "Join a Self Help Group (SHG) for group lending",
                "Consider microfinance institutions with flexible criteria",
                "Work on improving income stability",
                "Revisit loan application after 6-12 months"
            ]
        
        # Add lender-specific steps if available
        if lender_recommendations and lender_recommendations.get('recommendations'):
            next_steps.append(f"Contact the {len(lender_recommendations['recommendations'])} recommended lenders shown on the map")
            next_steps.append("Visit the nearest lender branch for face-to-face consultation")
        
        return next_steps
    
    def _fallback_comprehensive_advice(self, context: Dict[str, Any]) -> str:
        """Fallback advice when AI generation fails"""
        
        risk_score = context['risk_assessment']['score']
        loan_amount = context['loan_request'].get('amount', 0)
        
        if risk_score >= 70:
            advice = f"""
            Good news! Your profile shows strong potential for loan approval.
            
            ✅ Recommended: Proceed with loan application for ₹{loan_amount:,}
            ...
            2. Contact recommended lenders in your area
            3. Compare offers from multiple lenders
            4. Submit application with complete documentation
            """
        elif risk_score >= 50:
            advice = f"""
            Your loan application has moderate approval chances with some conditions.
            
            ⚠️ Recommended: Consider a smaller amount or additional support
            
            Areas to strengthen:
            - Improve income documentation
            - Consider group lending options
            - Provide additional collateral if possible
            
            Next steps:
            1. Consider starting with a smaller loan amount
            2. Explore Self Help Group (SHG) options
            3. Contact specialized microfinance institutions
            """
        else:
            advice = f"""
            Your current profile needs improvement for loan approval.
            
            ❌ Current recommendation: Focus on profile building first
            
            Areas to improve:
            - Build consistent income history
            - Establish savings pattern
            - Consider joining financial groups
            
            Next steps:
            1. Join a Self Help Group (SHG)
            2. Build savings for 6-12 months
            3. Improve income stability
            4. Reapply after profile improvement
            """
